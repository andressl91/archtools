...
if [ $WITH_XTOOLS -ne 0 ]; then

    echo USING X-TOOLS
    if [ ! -d x-tools.tar.xz ]; then
        echo 1
    fi
    INSTALL_PATH=$ROOT_DIR/x-tools/$TARGET
    PACMAN_PACKAGE_ROOT=$INSTALL_PATH/$TARGET/sysroot

fi

echo $INSTALL_PATH
echo $PACMAN_PACKAGE_ROOT
